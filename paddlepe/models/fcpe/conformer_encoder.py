...
from paddlepe.models.fcpe.local_attention import LocalAttention
    ...
    def __init__(
        ...
        if conv_dropout > 0.0:
            self.conformer = nn.Sequential(
                ConformerConvModule(dim_model), nn.Dropout(conv_dropout)
            )
        else:
...
def softmax_kernel(
    ...
    *,
    ...
    if is_query:
        ...
        data_dash = ratio * (
            paddle.exp(data_dash - diag_data + eps)
