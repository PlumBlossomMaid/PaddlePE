...
def max_neg_value(tensor):
    try:
        ...
        res = -np.finfo(dtype).max
        return res


def l2norm(tensor):
    dtype = tensor.dtype
    normed = F.normalize(tensor, axis=-1)
...
class LocalAttention(nn.Layer):
    ...
    def forward(
        ...
        if autopad:
            ...
            (needed_pad, q), (_, k), (_, v) = (
                pad_to_multiple(t, self.window_size, dim=-2) for t in (q, k, v)
            )
        ...
        bq, bk, bv = (
            rearrange(t, 'b (w n) d -> b w n d', w=windows) for t in (q, k, v)
        )
