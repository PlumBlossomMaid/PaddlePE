...
from paddlepe.models.rmvpe.backbone import RMVPEUNet, N_MELS, N_CLASS
...
class MelSpectrogram(nn.Layer):
    ...
    def __init__(self, n_mels=128, sr=16000, n_fft=1024, hop_length=160, win_length=1024, f_min=0.0, f_max=8000.0):
        super().__init__()
        self.n_mels = n_mels
        self.sr = sr
        self.n_fft = n_fft
        ...
        hann = paddle.hann_window(win_length, dtype=paddle.float32)
...
class RMVPEPE(BasePE):
    ...
    def infer(
        ...
        **kwargs,
        ...
        if sr != self.sample_rate:
            ...
            wav = F.interpolate(wav.unsqueeze(-1).transpose([0, 2, 1]),
                                size=new_len, mode="linear").transpose([0, 2, 1]).squeeze(-1)
        ...
        if use_viterbi:
            ...
            f0_cent = paddle.sum(ct * latent, axis=-1) / (paddle.sum(latent, axis=-1) + 1e-10)
    ...
    def default_ckpt(cls) -> str:
        return str(Path(__file__).parent.parent.parent.parent / "ckpts" / "rmvpe.pdparams")
