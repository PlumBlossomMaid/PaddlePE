...
assert f0.shape[0] > 0
print(
    f"  ✅ pe.infer(numpy) → f0: {f0.shape}, conf: {conf.shape if conf is not None else None}"
)
...
try:
    ...
    print("  ⚠️  跳过 torch.Tensor 输入测试（这是 DLL 冲突的预期行为）")
...
print("  ✅ from paddlepe import registry 成功")
