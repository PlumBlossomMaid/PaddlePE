...
TORCH_ASSETS = PROJECT_ROOT.parent / "pes" / "torchcrepe" / "torchcrepe" / "assets"
...
def convert(
    ...
    keys_to_skip = {"conv1_BN.num_batches_tracked",
    ...
    for k, v in torch_ckpt.items():
        ...
        if k in keys_to_skip:
            print(f"  {k:40s} {str(shape):20s}  (skipping)")
        elif k in key_map:
            print(f"  {k:40s} {str(shape):20s} -> {key_map[k]}")
        else:
            print(f"  {k:40s} {str(shape):20s}  (no mapping)")
    ...
    for torch_key, torch_tensor in torch_ckpt.items():
        ...
        if torch_key == "classifier.weight":
            ...
            print(f"  (transposed for Paddle Linear weight format)")

        # Check shape compatibility
        if paddle_key in dict(paddle_model.named_parameters()):
            expected = list(dict(paddle_model.named_parameters())[paddle_key].shape)
    ...
    paddle.save(paddle_state_pe, str(output))  # Save with 'backbone.' prefix for CrepePE
    print(f"  Saved {len(paddle_state_pe)} parameters (with 'backbone.' prefix).")

    # Also save a bare version for CrepeBackbone directly
    bare_output = output.parent / output.name.replace(".pdparams", "_backbone.pdparams")
    ...
    if verify and not errors:
        _verify(torch_ckpt_path, paddle_model, paddle_state_pe, paddle_state, capacity)
...
def _verify(
    ...
    torch_ckpt = torch.load(torch_ckpt_path, map_location="cpu", weights_only=True)
    ...
    paddle_params = {k: paddle.to_tensor(v) for k, v in paddle_state_bare.items()}
